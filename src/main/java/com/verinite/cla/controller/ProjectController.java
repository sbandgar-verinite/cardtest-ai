...
//  
	...
	public List<ProjectDto> fetchAllProjects() {
		...
		return projects.stream().map(project -> {
			...
			List<IterationDto> iterationDtos = project.getIterations().stream()
					.map(iteration -> mapper.map(iteration, IterationDto.class)).sorted(Comparator
							.comparing(IterationDto::getSequence, Comparator.nullsLast(Comparator.naturalOrder())))
					.collect(Collectors.toList());
